...
capi_signature str_to_sig(const string& sig, const bool& checksumming = true) {
    ...
    eosio_assert(pivot != string::npos, "No delimiter in capi_signature");
    ...
    eosio_assert(next_pivot != string::npos, "No curve in capi_signature");
    ...
    eosio_assert(decode_base58(data_str, vch), "Decode capi_signature failed");

    eosio_assert(vch.size() == 69, "Invalid capi_signature");
